...
// --------------------------------------------------------
void RayTracing::CreateTopLevelAccelerationStructureForScene(std::vector<std::shared_ptr<GameObject>> scene)
{
	...
	if (sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * instDescs.size() > tlasInstanceDataSizeInBytes[frameIndex])
	{
		...
		tlasInstanceDataSizeInBytes[frameIndex] = sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * instDescs.size();
	...
	memcpy(mapped, &instDescs[0], sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * instDescs.size());
	...
	accelStructInputs.NumDescs = (unsigned int)instDescs.size();
	accelStructInputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE;
...
void RayTracing::Raytrace(std::shared_ptr<Camera> camera, Microsoft::WRL::ComPtr<ID3D12Resource> currentBackBuffer)
{
	...
	D3D12_GPU_DESCRIPTOR_HANDLE cbuffer = Graphics::FillNextConstBufAndGetGPUDescHan(&sceneData, sizeof(RaytracingSceneData));
	...
	// ACTUAL RAYTRACING HERE
	{
		...
		DXRCommandList->SetComputeRoot32BitConstants(
			0, sizeof(RayTracingDrawData) / sizeof(unsigned int), &data, 0);
...
// --------------------------------------------------------
void RayTracing::CreateEntityDataBuffer(std::vector<std::shared_ptr<GameObject>> scene)
{
	...
	for (int i = 0; i < scene.size(); i++)
	{
		...
		DirectX::XMFLOAT3 c = scene[i]->GetMaterial()->GetTintColor();
